...
r_POS1      = 1
r_POS2      = 2
_r_DEST     = 3 #added the preceeding underscore as a reminder that this is just the did from now on
...
#             if numreads_ < self.min_reads: ##TODO: Is this any different during the iterative stages?
#                 continue
    ...
    def get_full_delta(self, pi, theta1, epsilon, use_theta):
        ...
        for rrid, mpngs in self._ambig_rds.items():
            ...
            if METHOD_OLD:
                noms = tuple(v for v in noms if v[3] >= epsilon)
            else:
...
def _add_read(covmap, pos1, pos2, avg_read_length, koef, length):
    numbins = len(covmap) #@@better to save this in a separate struct
    flank_size = _get_flank_size(length, numbins, avg_read_length)
    binlength = (length - (flank_size * 2.))/(numbins - 2)# Make safe by checking existence in maps??
...
def _calc_qual(sr, pmap_map, seqdict):
    if hasattr(pmap_map, 'map_quality'):
        return 10**(pmap_map.map_quality), 0, None #10**(pmap_map.map_quality/10), 0, None
    elif hasattr(pmap_map, 'map_quality1'):
        return 10**((pmap_map.map_quality1 + pmap_map.map_quality2)), 0, 0 #10**((pmap_map.map_quality1 + pmap_map.map_quality2)/10), 0, 0
    elif hasattr(pmap_map, 'mismatches'):
