...
import gzip
import ujson
import Data.Mapping.cy_ext_seq.sam2pmp_helper as s2ph
...
def sam2pmp(samfile, pmpfile, full = True):
    ...
    with gzip.open(pmpfile, 'wt') as of:
        for _, grp in groupby(pysam.AlignmentFile(samfile), attrgetter('query_name')):  # @UndefinedVariable
            alngrp = list(grp)
...
def _se_from_aln(aln, unique, full):
    ...
    if full:
        ...
        return DestMap_se(aln.reference_name.split()[0], nm, _get_strand(aln), aln.pos,
                          aln.cigarstring, _get_pe_end(aln), md, asc, qual)
    return DestMap_se_part(aln.reference_name.split()[0], dict(aln.tags)['NM'], _get_strand(aln), 
...
class SourceReadSAM(object):
    ...
    def from_alngroup(cls, alngrp, full):
        fst = alngrp[0]
        res = cls(fst.query_name, [fst.query_sequence], [fst.query_qualities], full)
        read1 = None
        ...
        for aln in alngrp:
            if not seen_read1 and aln.is_read1:
                ...
                res.quals.insert(0, aln.query_qualities)
                seen_read1 = True
            if not seen_read2 and aln.is_read2:
                res.seq.append(aln.query_sequence)
                res.quals.append(aln.query_qualities)
                seen_read2 = True
