...
#### designated functions - LMM ocean microbiome

#Craete 'phenotype' files 
create_pheno_files <- function(Data, ind, path_to_save){
  ...
  for(i in ind){
    ...
    y_train_vec = Data[,i] 
    phen_file = matrix(NA, ncol = 3, nrow = length(y_train_vec))
    id = c(1:length(y_train_vec))
    
    phen_file[,1] = rep(0,  length(y_train_vec))
    phen_file[,2] = id
    phen_file[,3] = y_train_vec
    
    write.table(phen_file, paste("phen_file__",i,".txt", sep = ""), col.names = FALSE, row.names = FALSE)
  }
  
}

#Craete Fixed effects files
create_Fixed_effect_files <- function(Data, path_to_save){
  
  setwd(path_to_save)
  
  dir.create("Fixed_effects") ## create a directory for output files
  setwd(paste(path_to_save, "Fixed_effects", sep = ""))
  
  prev_t = data.frame(rep(0, dim(Data)[1]), c(1:dim(Data)[1]), Data)
  
  write.table(prev_t, paste("Fixed_effects.txt", sep = ""), col.names = FALSE, row.names = FALSE)
  ...
  if(prediction_flag == 1){
    ...
    for(k in 1:length(t_data)){
      ...
      for(i in 1:num_GRM){
        ...
        GRM[k,i] = paste("GRM_files_pred/GRM_3_bins__",i,"_",t_data[k], sep = "")
        tmp = rbind(tmp, GRM[k,i]) 
      }
    ...
    for(i in 1:num_GRM){
      ...
      GRM[1,i] = paste("GRM_files/GRM_",i, sep = "")
      tmp = rbind(tmp, GRM[1,i]) 
    }
...
Create_hsq_data_Fixed_effects <- function(path_hsq_files, hsq_txt, num_time_points, var_comp_num, 
                                          Refactor_flag, data=NULL, individuals = All_individuals, 
                                          Fixed_effect_flag = 1, 
                                          config = c("1_GRM_FF", "1_GRM", "2_GRM_FF", "2_GRM"), numer_of_fixed_effects){
  
  
  # path_hsq_files = path_hsq_files;
  # hsq_txt = "GRM_OTUs_reml__";
  # num_time_points = N;
  # config = config;
  # Fixed_effect_flag  = Fixed_effect_flag;
  ...
  # path_hsq_files = paste0(dir_path,"/", TE_path);
  # Data = scaled.dat;
  # All_individuals = dim(scaled.dat)[1];
  # num_time_points = 1;
  # Fixed_effect_flag = fixed_effect_flag;
  # ind_1 = NULL;
  # config = config_flag
  ...
  Time_points_data = list()
  
  k = 1
  for(i in na.omit(index)){
    
    Time_points_data[[k]] = as.matrix(read.table(paste(hsq_txt ,i,".hsq", sep = ""), 
                                                 sep = "\t", fill = T, header = T))
  ...
  if(config == "1_GRM_FF"){
    ...
    for(j in 1:length(index)){
      ...
      hsq_time_index = which(Time_points_data[[j]][,1] == "V(G)/Vp")
      hsq_time[j] = as.numeric(Time_points_data[[j]][hsq_time_index,2])
      se_hsq_time[j] = as.numeric(Time_points_data[[j]][hsq_time_index,3])
      
      fixed_tmp = which(Time_points_data[[j]][,1] == "Fix_eff")
      num_FE = dim(Time_points_data[[j]])[1] - fixed_tmp
      fixed_effect_list[[j]] = as.numeric(Time_points_data[[j]][c((fixed_tmp+2) : (fixed_tmp+num_FE)),1])
      intercept[j] = as.numeric(Time_points_data[[j]][(fixed_tmp+1),1])
      
      p_val_index = which(Time_points_data[[j]][,1] == "Pval")
      p_val_hsq[j] = as.numeric(Time_points_data[[j]][p_val_index,2])
      
      logL_index = which(Time_points_data[[j]][,1] == "logL")
      logL[j] = as.numeric(Time_points_data[[j]][logL_index,2])
      
      logL0_index = which(Time_points_data[[j]][,1] == "logL0")
      logL0[j] = as.numeric(Time_points_data[[j]][logL0_index,2])
      
      
    }
    
    fixed_effect = matrix(unlist(fixed_effect_list), ncol = length(fixed_effect_list[[1]]), nrow = length(fixed_effect_list), byrow = T)
    colnames(fixed_effect) = c(1: length(fixed_effect_list[[1]]))
    
    hsq_data = data.frame( hsq_time, se_hsq_time, p_val_hsq,intercept, fixed_effect, logL0, logL, index)
  }
  ...
  if(config == "1_GRM"){
    ...
    hsq_data = data.frame( hsq_time, se_hsq_time,  p_val_hsq, logL0, logL, index)
  }
  ...
  if(config == "2_GRM"){
    ...
    for(j in 1:length(index)){
      ...
      hsq_time_index = which(Time_points_data[[j]][,1] == "V(G1)/Vp")
      hsq_time_1[j] = as.numeric(Time_points_data[[j]][hsq_time_index,2])
      se_hsq_time_1[j] = as.numeric(Time_points_data[[j]][hsq_time_index,3])
      
      hsq_time_index_2 = which(Time_points_data[[j]][,1] == "V(G2)/Vp")
      hsq_time_2[j] = as.numeric(Time_points_data[[j]][hsq_time_index_2,2])
      se_hsq_time_2[j] = as.numeric(Time_points_data[[j]][hsq_time_index_2,3])
    ...
    hsq_data = data.frame( hsq_time_1, se_hsq_time_1, hsq_time_2, se_hsq_time_2,  p_val_hsq, logL0, logL, index)
  }
...
Calculate_VE_func <- function(path_hsq_files, hsq_txt_tmp, 
                              Data, All_individuals, num_time_points, 
                              Fixed_effect_flag = 0, var_comp_num, ind_1, config){
  ...
  hsq_data_NEW_Fixed_effects = Create_hsq_data_Fixed_effects(path_hsq_files = path_hsq_files,
                                                             hsq_txt = hsq_txt_tmp, 
                                                             config = config, Fixed_effect_flag  = Fixed_effect_flag,
                                                             individuals = 1)
